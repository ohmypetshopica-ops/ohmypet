...
document.addEventListener('DOMContentLoaded', () => {
    // Aseguramos que el DOM esté completamente cargado antes de seleccionar elementos
    ...
    // Elementos del nuevo modal unificado
    ...
    const openCompletionModal = async (appointmentId) => {
        ...
        // Mostrar spinners mientras se cargan las fotos
        ...
        arrivalPhotoContainer.innerHTML = arrivalPhoto 
            ? `<img src="${arrivalPhoto.image_url}" class="w-full h-full object-cover rounded-lg">` 
            : `<p class="text-sm text-gray-500 text-center p-4">Clic para subir foto de llegada</p>`;
        departurePhotoContainer.innerHTML = departurePhoto 
            ? `<img src="${departurePhoto.image_url}" class="w-full h-full object-cover rounded-lg">` 
            : `<p class="text-sm text-gray-500 text-center p-4">Clic para subir foto de salida</p>`;
    };
