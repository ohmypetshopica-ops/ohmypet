...
let cartModal, cartModalContent, cartButton, closeCartButton;
let cartItemsContainer, cartSubtotal, cartCountBadge, checkoutButton, addToCartNotification;
let deleteConfirmModal, modalCancelBtn, modalConfirmBtn;
        ...
        <div class="cart-item flex items-center gap-4 p-4 bg-white rounded-xl shadow-sm border border-gray-100 mb-3 hover:shadow-md transition">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-2">
                    ...
                    <input type="number" 
                           class="quantity-input w-14 text-center border border-gray-300 rounded-lg py-1 text-sm font-semibold" 
                           value="${item.quantity}" 
                           min="1" 
                           max="${item.stock}"
                           data-product-id="${item.id}">
                    
                    <button class="increase-btn bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold h-8 w-8 rounded-lg transition flex items-center justify-center" 
                            data-product-id="${item.id}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button class="remove-btn text-red-500 hover:text-red-700 p-2 rounded-lg hover:bg-red-50 transition" 
                    data-product-id="${item.id}">
...
const closeCart = () => {
    if (!cartModalContent || !cartModal) return;
...
const processCheckout = async () => {
    ...
    cart.forEach((item, index) => {
        message += `${index + 1}. ${item.name} - S/${item.price.toFixed(2)} x${item.quantity}\n`;
    });
...
export const setupCartEventListeners = () => {
    ...
    cartModalContent = document.querySelector('#cart-modal-content');
    cartButton = document.querySelector('#cart-button');
    closeCartButton = document.querySelector('#close-cart-button');
    ...
    addToCartNotification = document.querySelector('#add-to-cart-notification');
